...
import { FaBan, FaSearch, FaFileExport } from 'react-icons/fa'
...
const CancelledOrdersPage = () => {
  const [orders] = useState(
    Array(23).fill().map((_, i) => ({
      ...
      reason: ['Customer cancelled', 'Out of stock', 'Delivery issues', 'Payment failed'][i % 4],
      status: 'Cancelled'
    }))
  )
    ...
    <div className="p-6">
      ...
      <div className="flex items-center mb-6">
        ...
        <div className="flex flex-col">
          ...
          <p className="text-sm text-gray-500">View all cancelled orders</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {orders.map((order, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{order.reason}</td>
